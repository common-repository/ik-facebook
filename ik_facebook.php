...
class ikFacebook extends GoldPlugin
{
	...
	//initialize configuration for Aloha
	function setupAloha()
	{
		// load Aloha
		$plugin_title = $this->is_pro ? 'WP Social Pro' : 'WP Social';
		$top_level_menu = 'ikfb_configuration_options';
		...
		$aloha_config = array(
			...
			'tagline' => $plugin_title .' is the best way to add Facebook to your website.'
		);      
		$this->Aloha = new GP_Aloha($aloha_config);

		// add a filter to get our welcome page's content
		add_filter( "gp_aloha_welcome_page_content_{$top_level_menu}", array($this, 'get_welcome_template') );
	}
	
	// loads the welcome page template for aloha
	// Note: variables in scope here will also be available in the template
	...
	function run_activation_steps(){
		...
		register_activation_hook( __FILE__, array($this->Aloha, 'reset_welcome_screen') );   
	}
	...
	function rewrite_flush() {
		//first, register post types so we have data present for flush to achieve goal of making post accessible via permalink
		$this->create_post_types();
	...
	function add_js(){
		//masonry js
		add_action( 'wp_enqueue_scripts', array($this, 'wpsp_setup_masonry_js'));
	}
	...
	function create_post_types(){
		...
		$customFields = array();
		$customFields[] = array('name' => 'like_count', 'title' => 'Like Count', 'description' => 'How many times this post has been Liked on Facebook.', 'type' => 'text', 'readonly' => true);	
		
		$customFields[] = array('name' => 'caption', 'title' => 'Caption', 'description' => 'This is the Caption associated with the Item.', 'type' => 'text', 'readonly' => true);	
		$customFields[] = array('name' => 'description', 'title' => 'Description', 'description' => 'This is the Description associated with the Item.', 'type' => 'text', 'readonly' => true);	
		$customFields[] = array('name' => 'link', 'title' => 'Link', 'description' => 'This is the Link associated with the Item.', 'type' => 'text', 'readonly' => true);	
		$customFields[] = array('name' => 'item_type', 'title' => 'Item Type', 'description' => 'The type of Content in the Item (e.g. Video)', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'content_source', 'title' => 'Content Source', 'description' => 'The path to the Source of the Content (e.g. the Video)', 'type' => 'text', 'readonly' => true);		
		$customFields[] = array('name' => 'status_type', 'title' => 'Status Type', 'description' => 'This is the Status Type of Item.', 'type' => 'text', 'readonly' => true);		
		
		$customFields[] = array('name' => 'comment_count', 'title' => 'Comment Count', 'description' => 'How many comments this post has on Facebook.', 'type' => 'text', 'readonly' => true);	
		$customFields[] = array('name' => 'fbid', 'title' => 'Facebook Post ID', 'description' => 'The ID of this post on Facebook.  Changing this value may prevent this post from updating.', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'feed_id', 'title' => 'Facebook Profile ID', 'description' => 'The ID of the Facebook Profile this post is from. This helps identify posts that come from different feeds.', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'updated_time', 'title' => 'Updated Time', 'description' => 'The timestamp of when this post was last updated on Facebook.  Changing this value may prevent this post from updating.', 'type' => 'text', 'readonly' => true);	
		$customFields[] = array('name' => 'created_time', 'title' => 'Created Time', 'description' => 'The timestamp of when this post was created on Facebook.', 'type' => 'text', 'readonly' => true);
		
		
		$customFields[] = array('name' => 'start_time', 'title' => 'Event Start Time', 'description' => 'When this event starts.', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'end_time', 'title' => 'Event End Time', 'description' => 'When this event ends.', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'location', 'title' => 'Event Location', 'description' => 'Where this event is occurring.', 'type' => 'text', 'readonly' => true);
		$customFields[] = array('name' => 'timezone', 'title' => 'Event Timezone', 'description' => 'The timezone of this event.', 'type' => 'text', 'readonly' => true);
	...
	function ik_fb_setup_css() {						
		...
		if($this->is_pro){
			$ikfb_themes['ik_facebook_classic_light'] = 'include/css/classic_light.css';
			$ikfb_themes['ik_facebook_classic_dark'] = 'include/css/classic_dark.css';
	...
	//add front end js
	function wpsp_setup_masonry_js(){	
		wp_register_script(
			'masonryInit',
			plugins_url('include/js/masonry_init.js', __FILE__),
			array('masonry'),
			'2.1.6', 
			true
		);
	...
	function fetchUrl($url,$decode=false){		
		...
		if($decode){
			$decoded_data = json_decode($retData);
			
			//if the decoded data was JSON, return it
			//otherwise fall through and return the undecoded data
...
function add_wpsp_media_tab($tabs)
{
	return array();
//	$tabs['wp_social_fb'] = 'Facebook';
//	return $tabs;
	
	$newtab = array( 'wp_social_fb' => 'Facebook' );
    return array_merge( $tabs, $newtab );	
}

add_filter( 'media_upload_tabs', 'add_wpsp_media_tab' );
...
add_action( 'media_upload_tab_wp_social_fb', 'custom_media_upload_tab_content' );